...
function CasalTab({P,isDark,setIsDark,profile,setProfile,diary,setDiary,progress}){
  ...
  {sub==="perfil"&&<div style={{padding:"20px 16px"}}><div style={{background:P.card,borderRadius:20,padding:20,border:`1px solid ${LP.gold}22`,marginBottom:16}}><div style={{textAlign:"center",marginBottom:16}}><div style={{fontSize:52}}>{profile.avatar||"💑"}</div><div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap",marginTop:8}}>{["💑","💍","🙏","✝️","❤️","🌹"].map(e=><button key={e} onClick={()=>setProfile(p=>({...p,avatar:e}))} style={{background:profile.avatar===e?`${LP.gold}33`:"none",border:`1px solid ${profile.avatar===e?LP.gold:LP.gold+"33"}`,borderRadius:10,padding:"4px 8px",cursor:"pointer",fontSize:18}}>{e}</button>)}</div></div>{[{k:"he",l:"Nome dele"},{k:"she",l:"Nome dela"},{k:"wedding",l:"Data do casamento",t:"date"},{k:"saint",l:"Santo padroeiro"}].map(f=><div key={f.k} style={{marginBottom:14}}><div style={{fontSize:10,color:P.textMuted,letterSpacing:1.5,fontFamily:"'Lora',serif",textTransform:"uppercase",marginBottom:4}}>{f.l}</div><input type={f.t||"text"} value={profile[f.k]||""} onChange={e=>setProfile(p=>({...p,[f.k]:e.target.value}))} style={{width:"100%",background:P.cream,border:`1.5px solid ${LP.gold}33`,borderRadius:12,padding:"10px 14px",fontFamily:"'Lora',serif",fontSize:13,color:P.text,outline:"none",boxSizing:"border-box"}}/></div>)}</div>{dm!==null&&<div style={{background:`linear-gradient(135deg,${LP.burgundyDeep},${LP.burgundy})`,borderRadius:16,padding:"16px 20px",textAlign:"center"}}><div style={{fontSize:10,color:LP.goldLight,letterSpacing:2,fontFamily:"'Lora',serif",textTransform:"uppercase"}}>Unidos há</div><div style={{fontSize:42,fontFamily:"'Playfair Display',serif",color:LP.gold,fontWeight:900,lineHeight:1}}>{dm.toLocaleString("pt-BR")}</div><div style={{fontSize:13,color:"#ffffff77",fontFamily:"'Lora',serif"}}>dias de aliança</div></div>}</div>}
  ...
  {sub==="distancia"&&<div style={{padding:"20px 16px"}}>
    <div style={{background:P.card,borderRadius:20,padding:"16px 12px",border:`1px solid ${LP.gold}22`,marginBottom:14}}>
      <div style={{textAlign:"center"}}><svg width="280" height="165" viewBox="0 0 320 190" style={{overflow:"visible",display:"block",margin:"0 auto"}}>
        <line x1="50" y1="172" x2="270" y2="172" stroke={LP.goldLight} strokeWidth="1.5" strokeDasharray="5 4" opacity="0.6"/>
        {[{x:(160-(100*(1-figP)/2)-24),he:true},{x:(160+(100*(1-figP)/2)+24-32),he:false}].map(({x,he},fi)=><g key={fi} style={{transform:`translateX(${x}px)`,transition:"transform 0.9s cubic-bezier(.34,1.56,.64,1)"}}><circle cx="16" cy="44" r="13" fill={he?LP.burgundy:"#8B2E3F"}/>{!he&&<ellipse cx="16" cy="35" rx="14" ry="9" fill="#5C1A2E"/>}<rect x="6" y={he?58:55} width={he?20:22} height={he?32:35} rx="6" fill={he?LP.burgundy:"#8B2E3F"}/><rect x="7" y="89" width="8" height="26" rx="4" fill={LP.burgundyDeep}/><rect x="17" y="89" width="8" height="26" rx="4" fill={LP.burgundyDeep}/><text x="16" y="130" textAnchor="middle" fontSize="9" fill={LP.textMuted} fontFamily="'Lora',serif">{he?"Ele":"Ela"}</text></g>)}
        {figP>0.08&&figP<0.97&&<line x1={160-(100*(1-figP)/2)} y1={108} x2={160+(100*(1-figP)/2)} y2={108} stroke={LP.goldLight} strokeWidth="2.5" strokeDasharray="4 3" opacity={Math.min(figP*2,0.8)}/>}
        {figP>0.25&&<g transform={`translate(160,28) scale(${figP>0.85?1+(figP-0.85)*4:figP*0.9})`} style={{opacity:Math.min(figP*1.5,1)}}><path d="M0,-9 C0,-16 -14,-16 -14,-7 C-14,3 0,14 0,14 C0,14 14,3 14,-7 C14,-16 0,-16 0,-9 Z" fill={LP.gold}/></g>}
      </svg></div>
...
export default function App(){
  ...
  return(<div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",background:P.cream,fontFamily:"'Lora',serif",position:"relative",transition:"background 0.3s"}}>
    <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,700;0,900;1,400&family=Lora:ital,wght@0,400;0,500;1,400&family=Cormorant+Garamond:ital,wght@0,400;0,500;1,400;1,500;1,600&display=swap" rel="stylesheet"/>
    ...
    {!selectedDay&&(<>
      {/* ── HOME ── */}
      {tab==="home"&&<div style={{paddingBottom:100}}>
        {/* Hero header */}
      ...
      {/* JOURNEY */}
      {tab==="journey"&&<div style={{paddingBottom:100}}><div style={{background:`linear-gradient(160deg,${LP.burgundyDeep},${LP.burgundy})`,padding:"52px 24px 24px",textAlign:"center"}}><CrossIcon size={28} color={LP.gold}/><div style={{fontSize:10,color:LP.goldLight,letterSpacing:3,fontFamily:"'Lora',serif",textTransform:"uppercase",marginTop:8}}>8 Dias</div><h2 style={{fontFamily:"'Playfair Display',serif",fontSize:26,color:"#fff",fontWeight:900,margin:"4px 0"}}>Propósitos Bíblicos</h2></div><div style={{paddingTop:18}}>{DAYS.map(day=><div key={day.id} onClick={()=>setSelectedDay(day)} style={{margin:"0 16px 10px",borderRadius:16,background:day.id===currentDay?`${day.color}18`:P.card,border:`1.5px solid ${day.id===currentDay?day.color:LP.gold+"22"}`,cursor:"pointer",boxShadow:day.id===currentDay?`0 4px 20px ${day.color}33`:`0 2px 8px ${P.shadow}`}}><div style={{display:"flex",alignItems:"center",padding:"14px 16px",gap:14}}><div style={{width:46,height:46,borderRadius:12,flexShrink:0,background:completedDays.includes(day.id)?day.color:day.id===currentDay?`${day.color}22`:"#F0E8D8",display:"flex",alignItems:"center",justifyContent:"center",fontSize:completedDays.includes(day.id)?18:22,color:"#fff"}}>{completedDays.includes(day.id)?"✓":day.icon}</div><div style={{flex:1}}><div style={{fontSize:10,color:P.textMuted,fontFamily:"'Lora',serif",letterSpacing:1.5,textTransform:"uppercase"}}>{day.label}</div><div style={{fontSize:16,fontFamily:"'Playfair Display',serif",color:P.burgundy,fontWeight:700,lineHeight:1.2}}>{day.sublabel}</div><div style={{fontSize:11,color:P.textMuted,fontFamily:"'Lora',serif",marginTop:2}}>{day.theme}</div></div>{day.id===currentDay&&<div style={{fontSize:9,background:day.color,color:"#fff",padding:"3px 9px",borderRadius:20,fontFamily:"'Lora',serif",flexShrink:0}}>HOJE</div>}{completedDays.includes(day.id)&&<div style={{fontSize:9,background:"#2E5E3A22",color:"#2E5E3A",padding:"3px 9px",borderRadius:20,fontFamily:"'Lora',serif",flexShrink:0}}>FEITO</div>}</div></div>)}</div></div>}
      ...
      {/* Nav */}
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,background:isDark?`${DP.white}F5`:`${LP.white}F2`,backdropFilter:"blur(14px)",borderTop:`1px solid ${LP.gold}33`,display:"flex",padding:"8px 0 16px",zIndex:40}}>
