...
function CasalTab({P,isDark,setIsDark,profile,setProfile,diary,setDiary,progress}){
  ...
  {sub==="perfil"&&<div style={{padding:"20px 16px"}}><div style={{background:P.card,borderRadius:20,padding:20,border:`1px solid ${LP.gold}22`,marginBottom:16}}><div style={{textAlign:"center",marginBottom:16}}><div style={{display:"flex",justifyContent:"center",marginBottom:4}}><span style={{fontSize:48,letterSpacing:-2}}>👨</span><span style={{fontSize:36,verticalAlign:"middle",margin:"0 2px"}}>❤️</span><span style={{fontSize:48,letterSpacing:-2}}>👩</span></div><div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap",marginTop:6}}>{["💍","🙏","✝️","❤️","🌹"].map(e=><button key={e} onClick={()=>setProfile(p=>({...p,avatar:e}))} style={{background:profile.avatar===e?`${LP.gold}33`:"none",border:`1px solid ${profile.avatar===e?LP.gold:LP.gold+"33"}`,borderRadius:10,padding:"4px 8px",cursor:"pointer",fontSize:18}}>{e}</button>)}</div></div><div style={{marginBottom:16,padding:"12px 14px",borderRadius:14,background:`${LP.gold}11`,border:`1px solid ${LP.gold}33`}}>
          ...
          <div style={{display:"flex",gap:8}}>
            {[{v:"casados",label:"💑 Casados",desc:"Jornada de renovação"},{v:"noivos",label:"💍 Noivos",desc:"Jornada de preparação"}].map(opt=>(
...
function LiturgyWidget({P}){
  ...
  const corMap={"Branco":"#D4AF37","Vermelho":"#C0392B","Verde":"#27AE60","Roxo":"#8E44AD","Rosa":"#E91E8C","Preto":"#2C1810"};
  ...
  return(
    <div style={{margin:"0 16px 4px",borderRadius:18,overflow:"hidden",boxShadow:`0 4px 20px ${cor}22`}}>
      <div onClick={()=>!loading&&!err&&setOpen(o=>!o)} style={{background:`linear-gradient(135deg,${cor}DD,${cor}88)`,padding:"12px 16px",display:"flex",alignItems:"center",gap:10,cursor:loading||err?"default":"pointer"}}>
        <div style={{width:34,height:34,borderRadius:"50%",background:"#ffffff22",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>
          {loading?"⏳":err?"📖":"✝️"}
        </div>
        <div style={{flex:1}}>
          <div style={{fontSize:9,color:"#ffffff88",letterSpacing:2,fontFamily:"'Lora',serif",textTransform:"uppercase"}}>Liturgia do Dia · {today}</div>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:"#fff",fontWeight:700,lineHeight:1.2}}>
            {loading?"Carregando...":err?"Sem conexão com liturgia":data?.liturgia||"Tempo Litúrgico"}
          </div>
          {data?.cor&&<div style={{fontSize:9,color:"#ffffff77",fontFamily:"'Lora',serif",marginTop:2}}>Cor litúrgica: {data.cor}</div>}
        </div>
        {!loading&&!err&&<span style={{fontSize:12,color:"#ffffff66",transition:"transform 0.25s",display:"inline-block",transform:open?"rotate(90deg)":"none"}}>›</span>}
      </div>

      <div style={{maxHeight:open?1200:0,overflow:"hidden",transition:"max-height 0.45s cubic-bezier(.4,0,.2,1)",background:P.card}}>
        {data&&[
          {label:"Primeira Leitura",d:data.primeiraLeitura},
          {label:"Salmo",d:data.salmo},
          {label:"Segunda Leitura",d:data.segundaLeitura},
          {label:"Evangelho",d:data.evangelho},
        ].filter(r=>r.d).map((r,i)=>(
          <div key={i} style={{padding:"14px 16px",borderTop:i>0?`1px solid ${LP.gold}18`:"none"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
              <div style={{background:cor,borderRadius:8,padding:"2px 10px"}}>
                <span style={{fontSize:9,color:"#fff",fontFamily:"'Lora',serif",letterSpacing:1}}>{r.label}</span>
              </div>
              <span style={{fontSize:10,color:P.textMuted,fontFamily:"'Lora',serif"}}>{r.d.referencia||r.d.refrao}</span>
            </div>
            {r.d.texto&&(
              <p style={{margin:0,fontFamily:"'Cormorant Garamond',serif",fontSize:14,color:P.text,lineHeight:1.8,fontStyle:"italic",
        ...
        {err&&(
          <div style={{padding:"16px",textAlign:"center"}}>
            <p style={{fontFamily:"'Lora',serif",fontSize:12,color:P.textMuted,margin:"0 0 10px",fontStyle:"italic"}}>
              Não foi possível carregar a liturgia. Acesse diretamente:
            </p>
            <a href="https://liturgia.up.railway.app" target="_blank" rel="noreferrer"
              style={{fontFamily:"'Lora',serif",fontSize:12,color:LP.gold}}>liturgia.up.railway.app →</a>
          </div>
        )}
      </div>
    </div>
  ...
  return(
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,${LP.burgundyDeep},${LP.burgundy})`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px"}}>
      ...
      <div style={{textAlign:"center",marginBottom:32}}>
        ...
        <div style={{fontSize:11,color:LP.goldLight,letterSpacing:4,fontFamily:"'Lora',serif",textTransform:"uppercase",marginTop:8}}>Casal em Fé</div>
...
export default function App(){
  ...
  return(<div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",minHeight:"100dvh",background:LP.cream,fontFamily:"'Lora',serif",position:"relative",transition:"background 0.3s",WebkitTapHighlightColor:"transparent",touchAction:"pan-y"}}>
    ...
    {!selectedDay&&(<>
      ...
      {/* JOURNEY */}
      {tab==="journey"&&(()=>{
        const isNoivos=(profile?.coupleType||"casados")==="noivos";
        const activeDays=isNoivos?NOIVOS_DAYS:DAYS;
        ...
        <div style={{paddingBottom:"calc(80px + env(safe-area-inset-bottom))"}}>
          <div style={{background:`linear-gradient(160deg,${LP.burgundyDeep},${LP.burgundy})`,padding:"52px 24px 24px",textAlign:"center"}}>
            ...
            <div style={{fontSize:10,color:LP.goldLight,letterSpacing:3,fontFamily:"'Lora',serif",textTransform:"uppercase",marginTop:8}}>
              {isNoivos?"Preparação para o Matrimônio":"8 Dias de Renovação"}
            </div>
            <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:26,color:"#fff",fontWeight:900,margin:"4px 0"}}>
              {isNoivos?"Jornada dos Noivos":"Propósitos Bíblicos"}
            </h2>
