...
// ── Supabase client ───────────────────────────────────────────────────────────
const SB_URL = "https://bccrfqqdqtqwqsdbxpog.supabase.co";
const SB_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJjY3JmcXFkcXRxd3FzZGJ4cG9nIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Nzg2OTE2ODUsImV4cCI6MjA5NDI2NzY4NX0.Qrw5bQL2O-ehUf9luXfbGiK65RbXEQdk5IrPJrD1v3o";
...
function shareWA(v){window.open(`https://wa.me/?text=${encodeURIComponent(`"${v.t}" — ${v.b} ${v.c},${v.v} 📖 2emCristo`)}`,"_blank");}
...
function CasalTab({P,isDark,setIsDark,profile,setProfile,diary,setDiary,progress}){
  ...
  async function reqN(){if(!("Notification" in window))return;const p=await Notification.requestPermission();if(p==="granted"){setNotif(true);new Notification("2emCristo 🙏",{body:`Propósito do dia: ${DAYS[Math.min(progress,7)].sublabel}!`});}}
  ...
  return(<div style={{paddingBottom:"calc(80px + env(safe-area-inset-bottom))"}}><div style={{background:`linear-gradient(160deg,${LP.burgundyDeep},${LP.burgundy})`,padding:"48px 24px 0",textAlign:"center"}}><CrossIcon size={26} color={LP.gold}/><div style={{fontSize:10,color:LP.goldLight,letterSpacing:3,fontFamily:"'Lora',serif",textTransform:"uppercase",marginTop:6}}>Nosso Espaço</div><h2 style={{fontFamily:"'Playfair Display',serif",fontSize:24,color:"#fff",fontWeight:900,margin:"4px 0 16px"}}>2emCristo</h2><div style={{display:"flex",borderTop:`1px solid #ffffff22`}}>{SUBS.map(s=><button key={s.id} onClick={()=>setSub(s.id)} style={{flex:1,padding:"10px 0",background:sub===s.id?"#ffffff22":"none",border:"none",borderTop:sub===s.id?`2px solid ${LP.gold}`:"2px solid transparent",color:sub===s.id?"#fff":"#ffffff66",fontFamily:"'Lora',serif",fontSize:11,cursor:"pointer"}}>{s.icon}<br/>{s.label}</button>)}</div></div>
  {sub==="perfil"&&<div style={{padding:"20px 16px"}}><div style={{background:P.card,borderRadius:20,padding:20,border:`1px solid ${LP.gold}22`,marginBottom:16}}><div style={{textAlign:"center",marginBottom:16}}><div style={{display:"flex",justifyContent:"center",marginBottom:4}}><span style={{fontSize:48,letterSpacing:-2}}>👨</span><span style={{fontSize:36,verticalAlign:"middle",margin:"0 2px"}}>❤️</span><span style={{fontSize:48,letterSpacing:-2}}>👩</span></div><div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap",marginTop:6}}>{["💍","🙏","✝️","❤️","🌹"].map(e=><button key={e} onClick={()=>setProfile(p=>({...p,avatar:e}))} style={{background:profile.avatar===e?`${LP.gold}33`:"none",border:`1px solid ${profile.avatar===e?LP.gold:LP.gold+"33"}`,borderRadius:10,padding:"4px 8px",cursor:"pointer",fontSize:18}}>{e}</button>)}</div></div><div style={{marginBottom:16,padding:"12px 14px",borderRadius:14,background:`${LP.gold}11`,border:`1px solid ${LP.gold}33`}}>
          <div style={{fontSize:10,color:P.textMuted,letterSpacing:1.5,fontFamily:"'Lora',serif",textTransform:"uppercase",marginBottom:10}}>Vocês são</div>
          <div style={{display:"flex",gap:8}}>
            {[{v:"casados",label:"💑 Casados",desc:"Jornada de renovação"},{v:"namorados",label:"🌹 Namorados",desc:"Jornada de discernimento"},{v:"noivos",label:"💍 Noivos",desc:"Jornada de preparação"}].map(opt=>(
              <div key={opt.v} onClick={()=>setProfile(p=>({...p,coupleType:opt.v}))} style={{flex:1,padding:"10px 10px",borderRadius:12,cursor:"pointer",textAlign:"center",
                background:(profile.coupleType||"casados")===opt.v?`linear-gradient(135deg,${LP.burgundy},${LP.burgundy}BB)`:`${LP.gold}11`,
                border:`1.5px solid ${(profile.coupleType||"casados")===opt.v?LP.burgundy:LP.gold+"44"}`,
                transition:"all 0.2s"}}>
                <div style={{fontSize:18,marginBottom:3}}>{opt.label.split(" ")[0]}</div>
                <div style={{fontSize:11,fontFamily:"'Playfair Display',serif",color:(profile.coupleType||"casados")===opt.v?"#fff":P.burgundy,fontWeight:700}}>{opt.label.split(" ").slice(1).join(" ")}</div>
                <div style={{fontSize:9,color:(profile.coupleType||"casados")===opt.v?"#ffffff88":P.textMuted,fontFamily:"'Lora',serif",marginTop:2}}>{opt.desc}</div>
              </div>
            ))}
          </div>
        </div>
...
// ── Onboarding ────────────────────────────────────────────────────────────────
const ONBOARD_STEPS=[
  {icon:"✝️",title:"Bem-vindos ao\n2emCristo",body:"Um espaço sagrado para os dois crescerem juntos na fé, no amor e na missão que Deus preparou para vocês.",cta:"Começar",bg:"#3B0F1D",accent:"#C8973A"},
...
function VerseOfDay({P}){
  ...
  function share(){const t=`"${v.text}" — ${v.ref} 📖 2emCristo`;window.open(`https://wa.me/?text=${encodeURIComponent(t)}`,"_blank");}
...
// ── Noivos Journey (8 dias para noivos) ──────────────────────────────────────
const NOIVOS_DAYS=[
  {id:0,label:"Preparação",sublabel:"Discernimento",icon:"💍",theme:"O Chamado ao Matrimônio",
    verse:'"Não é bom que o homem esteja só; farei uma auxiliar que lhe seja semelhante." — Gn 2,18',
    reflection:"Antes de tudo, sentem-se e perguntem: é Deus quem nos une? O matrimônio é uma vocação — não apenas uma decisão humana. Este dia é para abrir o coração ao que Deus quer.",
    propositos:["Rezem juntos: 'Senhor, é a Tua vontade que nos casemos?'","Compartilhem o momento em que cada um sentiu certeza sobre o outro","Escrevam as virtudes que admiram um no outro","Guardem o papel — releiam na véspera do casamento"],color:"#7A4A6A"},
  {id:1,label:"Dia 1",sublabel:"Vocação",icon:"✝️",theme:"Casamento como Sacramento",
    verse:'"Este mistério é grande — refiro-me a Cristo e à Igreja." — Ef 5,32',
    reflection:"O matrimônio não é apenas um ato civil. É um sacramento — sinal visível da graça de Deus. Vocês dois serão sinal um do outro para sempre.",
    propositos:["Leiam juntos o rito do Matrimônio e meditem nos votos","Perguntem: 'O que me atrai em você além do físico?'","Conversem sobre a vida sacramental que desejam construir","Releiam a homilia de um casamento que os emocionou"],color:"#5C1A2E"},
  {id:2,label:"Dia 2",sublabel:"Pureza",icon:"🌸",theme:"Castidade: Dom e Preparação",
    verse:'"Honrado seja o matrimônio em todos, e o leito nupcial seja imaculado." — Hb 13,4',
    reflection:"A castidade pré-matrimonial não é repressão — é respeito profundo. É dizer ao outro: 'Você vale a espera. Eu me preparo para te amar com todo o meu ser.'",
    propositos:["Conversem sobre os limites que honrarão até o casamento","Rezem juntos pedindo graça para guardar a castidade","Estudem a Teologia do Corpo de João Paulo II","Criem um plano para evitar situações de tentação"],color:"#8B2E3F"},
  {id:3,label:"Dia 3",sublabel:"Oração",icon:"🙏",theme:"Rezar antes de Tudo",
    verse:'"Se dois de vós concordarem na terra acerca de qualquer coisa que pedirem, isso lhes será feito." — Mt 18,19',
    reflection:"O casal que aprende a rezar junto antes do casamento chega ao altar com uma intimidade espiritual que muitos casais demoram anos para construir.",
    propositos:["Criem um horário fixo de oração em casal","Rezem o terço juntos esta semana","Compartilhem como é a vida de oração pessoal de cada um","Criem uma oração própria do noivado — com suas palavras"],color:"#2E5E3A"},
  {id:4,label:"Dia 4",sublabel:"Comunicação",icon:"💬",theme:"Conversar de Verdade",
    verse:'"Sede prontos para ouvir, tardios para falar, tardios para a ira." — Tg 1,19',
    reflection:"Muitos casamentos enfrentam crises com raízes em conversas que não aconteceram antes do casamento. Aprendam a escuta ativa agora.",
    propositos:["Respondam: 'Qual é o meu maior medo sobre o casamento?'","Conversem sobre filhos: quantos, quando, como educar","Falem sobre finanças: como administrar o dinheiro juntos","Discutam expectativas sobre sogros e limites com famílias"],color:"#1A4A6E"},
  {id:5,label:"Dia 5",sublabel:"Origens",icon:"🏡",theme:"Famílias de Origem",
    verse:'"Por isso deixará o homem seu pai e sua mãe e se unirá à sua mulher." — Gn 2,24',
    reflection:"Cada noivo traz a história da sua família — seus dons e suas feridas. Conhecer a origem do outro é conhecer parte essencial de quem você vai se casar.",
    propositos:["Cada um conta ao outro a história da família de onde veio","Identifiquem padrões das famílias de origem que querem preservar","Identifiquem padrões que querem conscientemente mudar","Rezem pela cura das feridas familiares que cada um carrega"],color:"#6A4A10"},
  {id:6,label:"Dia 6",sublabel:"Projeto",icon:"🗺️",theme:"Projeto de Vida a Dois",
    verse:'"Onde irdes, irei eu; onde morreres, morrerei eu." — Rt 1,16',
    reflection:"Um casamento sólido começa com uma visão comum. Não basta se amar — é preciso caminhar na mesma direção, com os mesmos valores.",
    propositos:["Criem juntos o projeto de vida: onde morar, como trabalhar, como servir","Escrevam os 5 valores inegociáveis que vão reger o lar","Conversem sobre o papel da fé na família que vão construir","Criem um lema do casal baseado em uma passagem bíblica"],color:"#1A4A5E"},
  {id:7,label:"Dia 7",sublabel:"Aliança",icon:"💒",theme:"A Promessa que Farão",
    verse:'"O que Deus uniu, o homem não separe." — Mt 19,6',
    reflection:"Em breve pronunciarão os votos mais importantes de suas vidas. Meditem no peso e na beleza dessa promessa — para sempre, na saúde e na doença.",
    propositos:["Releiam os votos do matrimônio em voz alta um para o outro","Escrevam uma carta pessoal para o cônjuge que lerão no dia","Rezem juntos pela cerimônia e pela vida que começará","Agradeçam a Deus por terem se encontrado — é graça, não acidente"],color:"#7A2A5A"},
];


// ── Namorados Journey ─────────────────────────────────────────────────────────
const NAMORADOS_DAYS=[
  {id:0,label:"Preparação",sublabel:"Conhecendo-nos",icon:"🌱",theme:"Quem É Você de Verdade?",
    verse:'"O coração do homem examina o seu caminho, mas o Senhor é quem lhe dirige os passos." — Pv 16,9',
    reflection:"Um namoro saudável começa com autoconhecimento. Antes de se oferecer ao outro, é preciso saber quem você é, o que acredita e para onde está caminhando.",
    propositos:["Cada um escreva 5 palavras que melhor o descrevem","Compartilhem um sonho que ainda não contaram a ninguém","Falem sobre a fé: o que Deus significa para cada um?","Escrevam juntos: por que este relacionamento existe?"],color:"#3A6A2A"},
  {id:1,label:"Dia 1",sublabel:"Amor vs Paixão",icon:"❤️‍🔥",theme:"O que Sinto é Amor ou Paixão?",
    verse:'"O amor é sofredor, é benigno; o amor não é invejoso." — 1 Cor 13,4',
    reflection:"Paixão é emoção intensa mas instável. Amor é decisão, paciência e serviço. Num namoro cristão, o objetivo é cultivar amor real — não apenas sentimento.",
    propositos:["Listem 3 diferenças entre amor e paixão com exemplos da relação","Perguntem: 'Você me ama ou está apaixonado por mim?'","Identifiquem um momento em que agiram com amor — não apenas sentimento","Rezem pedindo discernimento sobre o que sentem um pelo outro"],color:"#8B2E3F"},
  {id:2,label:"Dia 2",sublabel:"Respeito",icon:"🤲",theme:"Limites que Protegem o Amor",
    verse:'"Que cada um saiba usar do seu próprio corpo com santidade e honra." — 1 Ts 4,4',
    reflection:"Limites não são muros — são expressão de respeito e cuidado. No namoro cristão, honrar o corpo do outro é um ato de amor profundo.",
    propositos:["Conversem abertamente sobre limites físicos e emocionais","Definam juntos o que 'respeitar um ao outro' significa na prática","Identifiquem situações que colocam pressão desnecessária","Rezem juntos pedindo pureza e integridade no relacionamento"],color:"#1A4A6E"},
  {id:3,label:"Dia 3",sublabel:"Comunicação",icon:"💬",theme:"Falar e Ouvir de Verdade",
    verse:'"Sede prontos para ouvir, tardios para falar." — Tg 1,19',
    reflection:"Relacionamentos morrem por falta de comunicação honesta. Aprender a dizer o que sente — sem manipular, sem guardar — é a base de tudo.",
    propositos:["Pratiquem: cada um fala 5 min sem interrupção sobre como está se sentindo","Perguntem: 'Existe algo que você quer me dizer mas tem medo?'","Identifiquem um padrão de comunicação que precisa mudar","Criem uma regra: nunca resolver conflito por mensagem"],color:"#2E5E3A"},
  {id:4,label:"Dia 4",sublabel:"Fé",icon:"🙏",theme:"Rezar Juntos muda Tudo",
    verse:'"Onde dois ou três estiverem reunidos em meu nome, ali estou eu." — Mt 18,20',
    reflection:"Casais que rezam juntos constroem sobre rocha. Começar o hábito de oração no namoro é investir no casamento antes mesmo de existir.",
    propositos:["Rezem o Pai Nosso de mãos dadas — talvez pela primeira vez","Compartilhem a experiência espiritual pessoal de cada um","Falem sobre praticar a fé juntos: Missa, confissão, retiros","Peçam a São Rafael Arcanjo a proteção do namoro"],color:"#7A4A6A"},
  {id:5,label:"Dia 5",sublabel:"Família",icon:"👨‍👩‍👧",theme:"Conhecer a Raiz do Outro",
    verse:'"Honra teu pai e tua mãe." — Ex 20,12',
    reflection:"Namorar alguém é, inevitavelmente, namorar a história da sua família. Quanto mais cedo vocês conhecerem as raízes um do outro, mais sólido será o chão.",
    propositos:["Cada um conta como foi a infância e o ambiente familiar","Compartilhem algo difícil que a família deixou como herança emocional","Conversem sobre como cada família lidava com conflitos","Rezem pelas famílias de origem de cada um"],color:"#6A4A10"},
  {id:6,label:"Dia 6",sublabel:"Sonhos",icon:"⭐",theme:"Para onde Vamos Juntos?",
    verse:'"Os planos do coração pertencem ao homem, mas a resposta da língua vem do Senhor." — Pv 16,1',
    reflection:"Sonhos incompatíveis geram sofrimento. Alinhá-los cedo não é romantismo — é responsabilidade. Vocês precisam saber se caminham para o mesmo lugar.",
    propositos:["Cada um compartilhe onde quer estar daqui a 5 e 10 anos","Identifiquem os pontos em que os sonhos se alinham e onde divergem","Conversem sobre casamento: é algo que ambos desejam?","Perguntem: 'Nosso namoro tem futuro de aliança?'"],color:"#1A4A5E"},
  {id:7,label:"Dia 7",sublabel:"Discernimento",icon:"🧭",theme:"É Para o Matrimônio?",
    verse:'"Se Deus é por nós, quem será contra nós?" — Rm 8,31',
    reflection:"Todo namoro cristão caminha para uma resposta: este é o caminho para o matrimônio ou não? Discernir com honestidade e fé é o ato mais maduro que um casal pode fazer.",
    propositos:["Cada um escreva: 'O que sinto sobre o futuro deste relacionamento?'","Compartilhem as respostas com honestidade e sem julgamento","Rezem juntos pedindo a Deus clareza sobre o caminho","Se houver dúvida, busquem um acompanhamento espiritual juntos"],color:"#4A3A8A"},
];
// ── Liturgy of the Day Widget ─────────────────────────────────────────────────
function LiturgyWidget({P}){
  ...
  useEffect(()=>{
    const cacheKey="liturgia-"+today;
    const cached=localStorage.getItem(cacheKey);
    if(cached){try{setData(JSON.parse(cached));setLoading(false);return;}catch{}}
    fetch("https://liturgia.up.railway.app/")
      .then(r=>r.json())
      .then(d=>{setData(d);localStorage.setItem(cacheKey,JSON.stringify(d));setLoading(false);})
      .catch(()=>{setErr(true);setLoading(false);});
  },[]);

  const corMap={"Branco":"#B8960C","Vermelho":"#C0392B","Verde":"#27AE60","Roxo":"#8E44AD","Rosa":"#D4479A","Preto":"#2C1810"};
  const cor=data?.cor?corMap[data.cor]||LP.burgundy:LP.burgundy;

  // Build readings list — filter nulls, also filter empty texto
  const readings=data?[
    {label:"1ª Leitura",icon:"📜",d:data.primeiraLeitura},
    {label:"Salmo",icon:"🎵",d:data.salmo},
    data.segundaLeitura&&Object.keys(data.segundaLeitura).length>0&&(data.segundaLeitura.texto||data.segundaLeitura.referencia)
      ?{label:"2ª Leitura",icon:"📖",d:data.segundaLeitura}:null,
    {label:"Evangelho",icon:"✝️",d:data.evangelho},
  ].filter(r=>r&&r.d&&(r.d.texto||r.d.referencia||r.d.refrao)):[];

  return(
    <div style={{margin:"0 16px 4px",borderRadius:18,overflow:"hidden",boxShadow:`0 4px 20px ${cor}22`}}>
      {/* Header */}
      <div onClick={()=>!loading&&!err&&setOpen(o=>!o)}
        style={{background:`linear-gradient(135deg,${cor}EE,${cor}88)`,padding:"12px 16px",
          display:"flex",alignItems:"center",gap:10,cursor:loading||err?"default":"pointer"}}>
        <div style={{width:34,height:34,borderRadius:"50%",background:"#ffffff22",
          display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>
          {loading?"⏳":err?"📖":"✝️"}
        </div>
        <div style={{flex:1}}>
          <div style={{fontSize:9,color:"#ffffff88",letterSpacing:2,fontFamily:"'Lora',serif",textTransform:"uppercase"}}>
            Liturgia do Dia · {today}
          </div>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:13,color:"#fff",fontWeight:700,lineHeight:1.2}}>
            {loading?"Carregando...":err?"Sem conexão":data?.liturgia||"Tempo Litúrgico"}
          </div>
          {data?.cor&&!loading&&<div style={{fontSize:9,color:"#ffffff77",fontFamily:"'Lora',serif",marginTop:1}}>
            Cor: {data.cor}{readings.length>0&&` · ${readings.length} leituras`}
          </div>}
        </div>
        {!loading&&!err&&(
          <span style={{fontSize:12,color:"#ffffff66",transition:"transform 0.25s",
            display:"inline-block",transform:open?"rotate(90deg)":"none"}}>›</span>
        )}
      </div>

      {/* Collapsible readings — horizontal scroll cards */}
      <div style={{maxHeight:open?500:0,overflow:"hidden",
        transition:"max-height 0.45s cubic-bezier(.4,0,.2,1)",background:P.card}}>

        {err&&(
          <div style={{padding:"16px",textAlign:"center"}}>
            <p style={{fontFamily:"'Lora',serif",fontSize:12,color:P.textMuted,margin:"0 0 8px",fontStyle:"italic"}}>
              Não foi possível carregar a liturgia.
            </p>
            <a href="https://liturgia.up.railway.app" target="_blank" rel="noreferrer"
              style={{fontFamily:"'Lora',serif",fontSize:12,color:LP.gold}}>
        ...
        {data&&readings.length>0&&(<>
          {/* Tab pills */}
          <div style={{display:"flex",gap:6,padding:"10px 14px 6px",overflowX:"auto",scrollbarWidth:"none"}}>
            {readings.map((r,i)=>(
              <button key={i} onClick={()=>setActiveCard(i)} style={{
                flexShrink:0,padding:"4px 12px",borderRadius:20,border:"none",cursor:"pointer",
                background:activeCard===i?cor:"transparent",
                color:activeCard===i?"#fff":P.textMuted,
                fontFamily:"'Lora',serif",fontSize:11,
                outline:activeCard===i?"none":`1px solid ${cor}44`,
                transition:"all 0.2s",
              }}>
                {r.icon} {r.label}
              </button>
          ...
          {/* Active reading card */}
          {readings[activeCard]&&(()=>{
            const r=readings[activeCard];
            const ref=r.d.referencia||r.d.refrao||"";
            const txt=r.d.texto||r.d.refrao||"";
            return(
              <div style={{padding:"10px 16px 16px"}}>
                {ref&&<div style={{fontSize:10,color:cor,fontFamily:"'Lora',serif",
                  fontWeight:"600",marginBottom:8,letterSpacing:0.5}}>{ref}</div>}
                {txt&&<p style={{margin:0,fontFamily:"'Cormorant Garamond',serif",
                  fontSize:15,color:P.text,lineHeight:1.85,fontStyle:"italic",
          ...
          {/* Scroll hint dots */}
          <div style={{display:"flex",gap:5,justifyContent:"center",paddingBottom:12}}>
            {readings.map((_,i)=>(
              <div key={i} onClick={()=>setActiveCard(i)} style={{
                width:i===activeCard?16:5,height:5,borderRadius:3,cursor:"pointer",
                background:i===activeCard?cor:`${cor}44`,transition:"all 0.25s",
              }}/>
            ))}
          </div>
        </>)}
...
function PrayerIntentionsList({P,intentions,setIntentions}){
  const[adding,setAdding]=useState(false);
  const[draft,setDraft]=useState("");
  const answered=intentions.filter(i=>i.status==="respondida").length;

  function add(){
    if(!draft.trim())return;
    const ni={id:Date.now()+"",text:draft.trim(),date:new Date().toLocaleDateString("pt-BR"),status:"orando"};
    setIntentions(old=>[ni,...old]);
    setDraft("");setAdding(false);
  }
  function toggle(id){
    setIntentions(old=>old.map(i=>i.id===id?{...i,status:i.status==="orando"?"respondida":"orando"}:i));
  }
  function remove(id){setIntentions(old=>old.filter(i=>i.id!==id));}

  return(
    <div style={{margin:"0 16px 4px",borderRadius:18,overflow:"hidden",boxShadow:`0 4px 16px ${LP.burgundy}18`}}>
      <div style={{background:`linear-gradient(135deg,#4A2A6A,#7A4A9A)`,padding:"12px 16px",display:"flex",alignItems:"center",gap:8}}>
        <span style={{fontSize:16}}>📿</span>
        <span style={{fontSize:9,color:"#E8D4F8",letterSpacing:2.5,fontFamily:"'Lora',serif",textTransform:"uppercase",flex:1}}>Intenções de Oração</span>
        <span style={{fontSize:9,color:"#ffffff66",fontFamily:"'Lora',serif"}}>{answered}/{intentions.length} respondidas</span>
      </div>
      <div style={{background:P.card,padding:"12px 14px"}}>
        {intentions.length===0&&!adding&&(
          <p style={{fontFamily:"'Lora',serif",fontSize:12,color:P.textMuted,fontStyle:"italic",margin:"0 0 10px",lineHeight:1.7}}>
            Nenhuma intenção ainda. Registre os pedidos do casal e acompanhe até serem respondidos.
          </p>
        )}
        {intentions.map(i=>(
          <div key={i.id} style={{display:"flex",alignItems:"flex-start",gap:10,marginBottom:10,padding:"10px 12px",borderRadius:12,
            background:i.status==="respondida"?`${LP.gold}11`:"#7A4A9A11",
            border:`1px solid ${i.status==="respondida"?LP.gold+"44":"#7A4A9A33"}`}}>
            <div onClick={()=>toggle(i.id)} style={{width:22,height:22,borderRadius:6,flexShrink:0,cursor:"pointer",marginTop:1,
              background:i.status==="respondida"?LP.gold:"none",
              border:`2px solid ${i.status==="respondida"?LP.gold:"#7A4A9A66"}`,
              display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s"}}>
              {i.status==="respondida"&&<span style={{fontSize:11,color:"#fff",fontWeight:"bold"}}>✓</span>}
            </div>
            <div style={{flex:1}}>
              <p style={{margin:"0 0 3px",fontFamily:"'Lora',serif",fontSize:12,color:i.status==="respondida"?P.textMuted:P.text,
                lineHeight:1.6,textDecoration:i.status==="respondida"?"line-through":"none"}}>{i.text}</p>
              <span style={{fontSize:9,color:P.textMuted,fontFamily:"'Lora',serif"}}>{i.date}</span>
            </div>
            <button onClick={()=>remove(i.id)} style={{background:"none",border:"none",color:P.textMuted,cursor:"pointer",fontSize:14,padding:0,flexShrink:0,opacity:0.5}}>×</button>
          </div>
        ))}
        {adding?(
          <>
            <textarea value={draft} onChange={e=>setDraft(e.target.value)} autoFocus placeholder="Ex: Pela saúde do papai. Pela paz na família. Pelo nosso casamento..."
              style={{width:"100%",minHeight:64,background:P.cream,border:`1.5px solid #7A4A9A55`,borderRadius:12,padding:"10px 12px",
                fontFamily:"'Lora',serif",fontSize:12,color:P.text,outline:"none",resize:"none",boxSizing:"border-box",lineHeight:1.7,marginBottom:8}}/>
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>{setAdding(false);setDraft("");}} style={{flex:1,background:"none",border:`1px solid ${LP.gold}33`,borderRadius:10,padding:"8px",cursor:"pointer",fontSize:11,color:P.textMuted,fontFamily:"'Lora',serif"}}>Cancelar</button>
              <button onClick={add} style={{flex:2,background:"linear-gradient(135deg,#4A2A6A,#7A4A9A)",border:"none",borderRadius:10,padding:"8px",cursor:"pointer",fontSize:11,color:"#fff",fontFamily:"'Lora',serif",fontWeight:"600"}}>📿 Adicionar intenção</button>
            </div>
          </>
        ):(
          <button onClick={()=>setAdding(true)} style={{width:"100%",background:"none",border:`1px dashed #7A4A9A55`,borderRadius:12,padding:"10px",cursor:"pointer",fontSize:11,color:"#7A4A9A",fontFamily:"'Lora',serif",marginTop:intentions.length>0?8:0}}>
...
// ── Auth helpers ──────────────────────────────────────────────────────────────
async function signUp(email,pw){return sb.auth.signUp({email,password:pw});}
async function signIn(email,pw){return sb.auth.signInWithPassword({email,password:pw});}
...
function LoginScreen({onAuth}){
  ...
  async function handle(){
    ...
    if(mode==="register"){setDone(true);return;}
    if(data.user) onAuth(data.user);
  }

  return(
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,${LP.burgundyDeep},${LP.burgundy})`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px"}}>
      ...
      <div style={{textAlign:"center",marginBottom:32}}>
        <svg width="32" height="32" viewBox="0 0 24 24"><rect x="10" y="2" width="4" height="20" rx="1.2" fill={LP.gold}/><rect x="3" y="8" width="18" height="4" rx="1.2" fill={LP.gold}/></svg>
        <div style={{fontSize:11,color:LP.goldLight,letterSpacing:4,fontFamily:"'Lora',serif",textTransform:"uppercase",marginTop:8}}>2emCristo</div>
        <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:32,color:"#fff",fontWeight:900,margin:"6px 0 4px",lineHeight:1.1}}>{mode==="login"?"Entrar":"Criar conta"}</h1>
        <p style={{color:"#ffffff77",fontSize:13,fontFamily:"'Lora',serif",fontStyle:"italic",margin:0}}>Continue sua jornada com seu cônjuge</p>
      </div>

      {done?(
        <div style={{background:LP.white,borderRadius:20,padding:28,width:"100%",maxWidth:340,textAlign:"center"}}>
          <div style={{fontSize:36,marginBottom:12}}>📧</div>
          <p style={{fontFamily:"'Playfair Display',serif",fontSize:18,color:LP.burgundy,fontWeight:700,margin:"0 0 8px"}}>Confirme seu e-mail</p>
          <p style={{fontFamily:"'Lora',serif",fontSize:13,color:LP.textMuted,lineHeight:1.7,margin:"0 0 16px"}}>Enviamos um link de confirmação para <strong>{email}</strong>. Acesse e depois volte aqui para entrar.</p>
          <button onClick={()=>{setDone(false);setMode("login");}} style={{background:`linear-gradient(135deg,${LP.burgundy},${LP.burgundy}BB)`,border:"none",borderRadius:14,padding:"12px 28px",color:"#fff",fontFamily:"'Playfair Display',serif",fontSize:14,fontWeight:700,cursor:"pointer"}}>Ir para Login</button>
        </div>
      ):(
        <div style={{background:LP.white,borderRadius:20,padding:28,width:"100%",maxWidth:340,boxShadow:"0 20px 60px #0004"}}>
          <div style={{marginBottom:16}}>
            <div style={{fontSize:10,color:LP.textMuted,letterSpacing:1.5,fontFamily:"'Lora',serif",textTransform:"uppercase",marginBottom:6}}>E-mail</div>
            <input value={email} onChange={e=>setEmail(e.target.value)} type="email" placeholder="seu@email.com" style={{width:"100%",background:LP.cream,border:`1.5px solid ${LP.gold}44`,borderRadius:12,padding:"12px 14px",fontFamily:"'Lora',serif",fontSize:13,color:LP.text,outline:"none",boxSizing:"border-box"}}/>
          </div>
          <div style={{marginBottom:err?8:20}}>
            <div style={{fontSize:10,color:LP.textMuted,letterSpacing:1.5,fontFamily:"'Lora',serif",textTransform:"uppercase",marginBottom:6}}>Senha</div>
            <input value={pw} onChange={e=>setPw(e.target.value)} type="password" placeholder="••••••••" onKeyDown={e=>e.key==="Enter"&&handle()} style={{width:"100%",background:LP.cream,border:`1.5px solid ${LP.gold}44`,borderRadius:12,padding:"12px 14px",fontFamily:"'Lora',serif",fontSize:13,color:LP.text,outline:"none",boxSizing:"border-box"}}/>
          </div>
          {err&&<p style={{fontSize:11,color:"#c0392b",fontFamily:"'Lora',serif",margin:"0 0 12px",lineHeight:1.5}}>{err}</p>}
          <button onClick={handle} disabled={loading||!email||!pw} style={{width:"100%",padding:"14px",background:loading||!email||!pw?"#ccc":`linear-gradient(135deg,${LP.burgundy},${LP.burgundy}BB)`,border:"none",borderRadius:14,color:"#fff",fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700,cursor:loading?"default":"pointer",marginBottom:14}}>
            {loading?"Aguarde...":(mode==="login"?"Entrar →":"Criar conta →")}
          </button>
          <div style={{display:"flex",alignItems:"center",gap:10,margin:"4px 0 8px"}}>
            <div style={{flex:1,height:1,background:`${LP.gold}33`}}/>
            <span style={{fontSize:10,color:LP.textMuted,fontFamily:"'Lora',serif"}}>ou</span>
            <div style={{flex:1,height:1,background:`${LP.gold}33`}}/>
          </div>
          <button onClick={()=>sb.auth.signInWithOAuth({provider:"google",options:{redirectTo:window.location.origin}})}
            style={{width:"100%",padding:"13px",background:"#fff",border:"1.5px solid #E0D0B8",borderRadius:14,
              display:"flex",alignItems:"center",justifyContent:"center",gap:10,cursor:"pointer",marginBottom:14,
              fontFamily:"'Lora',serif",fontSize:13,color:"#3A2A10",boxShadow:"0 2px 8px #0001"}}>
            <svg width="18" height="18" viewBox="0 0 48 48"><path fill="#EA4335" d="M24 9.5c3.5 0 6.6 1.2 9.1 3.2l6.8-6.8C35.8 2.4 30.3 0 24 0 14.7 0 6.7 5.4 2.7 13.3l7.9 6.1C12.5 13.1 17.8 9.5 24 9.5z"/><path fill="#4285F4" d="M46.6 24.5c0-1.6-.1-3.1-.4-4.5H24v8.5h12.7c-.6 3-2.3 5.5-4.8 7.2l7.5 5.8c4.4-4 6.9-10 6.9-17z"/><path fill="#FBBC05" d="M10.6 28.6A14.9 14.9 0 0 1 9.5 24c0-1.6.3-3.2.8-4.6l-7.9-6.1A23.9 23.9 0 0 0 0 24c0 3.9.9 7.5 2.5 10.8l8.1-6.2z"/><path fill="#34A853" d="M24 48c6.3 0 11.6-2.1 15.5-5.7l-7.5-5.8c-2.1 1.4-4.8 2.2-7.9 2.2-6.2 0-11.5-4.2-13.4-9.9l-8.1 6.2C6.7 42.6 14.7 48 24 48z"/><path fill="none" d="M0 0h48v48H0z"/></svg>
            Entrar com Google
          </button>
          <button onClick={()=>{setMode(m=>m==="login"?"register":"login");setErr("");}} style={{width:"100%",background:"none",border:"none",fontFamily:"'Lora',serif",fontSize:12,color:LP.textMuted,cursor:"pointer"}}>
...
// ── CoupleSetup ───────────────────────────────────────────────────────────────
function CoupleSetup({userId,onReady}){
  ...
  async function createCouple(){
    setLoading(true);setErr("");
    const{data:casal,error:e1}=await sb.from("casais").insert({}).select().single();
    if(e1){setErr(e1.message);setLoading(false);return;}
    const{error:e2}=await sb.from("membros").insert({user_id:userId,couple_id:casal.id});
    if(e2){setErr(e2.message);setLoading(false);return;}
    await sb.from("estado").insert({couple_id:casal.id});
    setMyCode(casal.codigo);
    setLoading(false);
  }

  async function joinCouple(){
    setLoading(true);setErr("");
    const{data:casal,error:e1}=await sb.from("casais").select().eq("codigo",code.toUpperCase().trim()).single();
    if(e1||!casal){setErr("Código não encontrado. Verifique e tente novamente.");setLoading(false);return;}
    const{error:e2}=await sb.from("membros").upsert({user_id:userId,couple_id:casal.id});
    if(e2){setErr(e2.message);setLoading(false);return;}
    setLoading(false);
    onReady(casal.id);
  }
  ...
  if(mode==="choose")return(
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,${LP.burgundyDeep},${LP.burgundy})`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px"}}>
      ...
      <div style={{textAlign:"center",marginBottom:32}}>
        ...
        <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:28,color:"#fff",fontWeight:900,margin:"0 0 8px"}}>Conectar o Casal</h1>
        <p style={{color:"#ffffff77",fontSize:13,fontFamily:"'Lora',serif",fontStyle:"italic",margin:0}}>Para sincronizar com seu cônjuge</p>
      </div>
      <div style={{width:"100%",maxWidth:340,display:"flex",flexDirection:"column",gap:12}}>
        <button onClick={()=>{createCouple();setMode("created");}} style={{padding:"16px",background:`linear-gradient(135deg,${LP.gold},${LP.warm})`,border:"none",borderRadius:16,color:LP.burgundyDeep,fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700,cursor:"pointer",boxShadow:`0 6px 20px ${LP.gold}44`}}>
          ✝️ Criar novo casal
        </button>
        <button onClick={()=>setMode("join")} style={{padding:"16px",background:"#ffffff22",border:"1.5px solid #ffffff44",borderRadius:16,color:"#fff",fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700,cursor:"pointer"}}>
          🔗 Entrar com código do cônjuge
        </button>
      </div>
    </div>
  );
  ...
  if(mode==="created")return(
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,${LP.burgundyDeep},${LP.burgundy})`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px"}}>
      <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:wght@700;900&family=Lora:ital,wght@0,400;1,400&display=swap" rel="stylesheet"/>
      {loading?(
        <div style={{textAlign:"center"}}><div style={{fontSize:32,marginBottom:12}}>⏳</div><p style={{color:"#fff",fontFamily:"'Lora',serif"}}>Criando seu casal...</p></div>
      ):(
        <div style={{textAlign:"center",width:"100%",maxWidth:340}}>
          <div style={{fontSize:36,marginBottom:12}}>🎉</div>
          <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:26,color:"#fff",fontWeight:900,margin:"0 0 8px"}}>Casal criado!</h2>
          <p style={{color:"#ffffff88",fontSize:13,fontFamily:"'Lora',serif",margin:"0 0 24px",lineHeight:1.7}}>Compartilhe este código com seu cônjuge para que ele entre no app e vocês se conectem:</p>
          <div style={{background:`${LP.gold}22`,border:`2px solid ${LP.gold}`,borderRadius:20,padding:"20px",marginBottom:24}}>
            <div style={{fontSize:9,color:LP.goldLight,letterSpacing:3,fontFamily:"'Lora',serif",textTransform:"uppercase",marginBottom:8}}>Código do Casal</div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:42,color:LP.gold,fontWeight:900,letterSpacing:8}}>{myCode}</div>
          </div>
          {err&&<p style={{fontSize:11,color:"#ffaaaa",fontFamily:"'Lora',serif",marginBottom:12}}>{err}</p>}
          <button onClick={async()=>{
            setLoading(true);
            const{data:casal}=await sb.from("casais").select("id").eq("codigo",myCode).single();
            if(casal) onReady(casal.id);
            setLoading(false);
          }} style={{width:"100%",padding:"14px",background:`linear-gradient(135deg,${LP.gold},${LP.warm})`,border:"none",borderRadius:14,color:LP.burgundyDeep,fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700,cursor:"pointer"}}>
            Entrar no App →
          </button>
        </div>
      )}
  ...
  return(
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,${LP.burgundyDeep},${LP.burgundy})`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px"}}>
      ...
      <div style={{textAlign:"center",marginBottom:28}}>
        ...
        <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:26,color:"#fff",fontWeight:900,margin:"0 0 6px"}}>Código do Casal</h2>
        <p style={{color:"#ffffff77",fontSize:13,fontFamily:"'Lora',serif",margin:0}}>Digite o código compartilhado pelo cônjuge</p>
      </div>
      <div style={{width:"100%",maxWidth:340}}>
        <input value={code} onChange={e=>setCode(e.target.value.toUpperCase())} maxLength={6} placeholder="ABC123" style={{width:"100%",background:"#ffffff22",border:"1.5px solid #ffffff44",borderRadius:16,padding:"16px",textAlign:"center",fontFamily:"'Playfair Display',serif",fontSize:32,color:"#fff",outline:"none",letterSpacing:8,boxSizing:"border-box",marginBottom:12}} />
        {err&&<p style={{fontSize:11,color:"#ffaaaa",fontFamily:"'Lora',serif",textAlign:"center",margin:"0 0 10px"}}>{err}</p>}
        <button onClick={joinCouple} disabled={loading||code.length<4} style={{width:"100%",padding:"14px",background:code.length>=4?`linear-gradient(135deg,${LP.gold},${LP.warm})`:"#ffffff22",border:"none",borderRadius:14,color:code.length>=4?LP.burgundyDeep:"#ffffff66",fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700,cursor:"pointer",marginBottom:12}}>
          {loading?"Buscando...":"Entrar no Casal →"}
        </button>
        <button onClick={()=>setMode("choose")} style={{width:"100%",background:"none",border:"none",color:"#ffffff55",fontFamily:"'Lora',serif",fontSize:12,cursor:"pointer"}}>← Voltar</button>
      </div>
    </div>
  );
}
...
export default function App(){
  ...
  useEffect(()=>{
    if(!user)return;
    sb.from("membros").select("couple_id").eq("user_id",user.id).single()
      .then(({data})=>{ if(data?.couple_id) setCoupleId(data.couple_id); });
  },[user]);
  ...
  const[prayerIntentions,setPrayerIntentions]=useStorage("pintentions",[]);
  const[tab,setTab]=useState("home");const[selectedDay,setSelectedDay]=useState(null);
  const[syncedOnce,setSyncedOnce]=useState(false);

  // ── Load from Supabase on couple connect ──────────────────────────────────
  useEffect(()=>{
    if(!coupleId||syncedOnce)return;
    sb.from("estado").select("*").eq("couple_id",coupleId).single()
      .then(({data})=>{
        if(!data)return;
        if(data.completed_days?.length>0) setCompleted(data.completed_days);
        if(data.start_date) setStartDateRaw(data.start_date);
        if(data.profile&&Object.keys(data.profile).length>0) setProfile(p=>({...p,...data.profile}));
        if(data.diary&&Object.keys(data.diary).length>0) setDiary(data.diary);
        if(data.theme_checked&&Object.keys(data.theme_checked).length>0) setThemeChecked(data.theme_checked);
        if(data.intention) setIntention(data.intention);
        if(data.favorites?.length>0) setFavorites(data.favorites);
        if(data.prayer_intentions?.length>0) setPrayerIntentions(data.prayer_intentions);
        setSyncedOnce(true);
      });
  },[coupleId]);
  ...
  useEffect(()=>{
    if(!coupleId)return;
    const t=setTimeout(()=>{
      sb.from("estado").upsert({
        couple_id:coupleId,
        completed_days:completedDays,
        start_date:startDate,
        profile,diary,
        theme_checked:themeChecked,
        intention,
        favorites,
    ...
    return()=>clearTimeout(t);
  },[coupleId,completedDays,startDate,profile,diary,themeChecked,intention,favorites,prayerIntentions]);

  // ── Realtime subscription ─────────────────────────────────────────────────
  useEffect(()=>{
    if(!coupleId)return;
    const ch=sb.channel("estado-"+coupleId)
      .on("postgres_changes",{event:"UPDATE",schema:"public",table:"estado",filter:"couple_id=eq."+coupleId},
        ({new:d})=>{
          if(d.completed_days) setCompleted(d.completed_days);
          if(d.start_date!==undefined) setStartDateRaw(d.start_date);
          if(d.profile) setProfile(p=>({...p,...d.profile}));
          if(d.diary) setDiary(d.diary);
          if(d.theme_checked) setThemeChecked(d.theme_checked);
          if(d.intention!==undefined) setIntention(d.intention);
          if(d.favorites) setFavorites(d.favorites);
          if(d.prayer_intentions) setPrayerIntentions(d.prayer_intentions);
        })
      .subscribe();
  ...
  // ── Auth gate ────────────────────────────────────────────────────────────────
  if(!authReady) return(
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,${LP.burgundyDeep},${LP.burgundy})`,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16}}>
      <link href="https://fonts.googleapis.com/css2?family=Lora:ital@1&display=swap" rel="stylesheet"/>
      <svg width="32" height="32" viewBox="0 0 24 24"><rect x="10" y="2" width="4" height="20" rx="1.2" fill="#C8973A"/><rect x="3" y="8" width="18" height="4" rx="1.2" fill="#C8973A"/></svg>
      <p style={{color:"#ffffff77",fontFamily:"'Lora',serif",fontStyle:"italic",fontSize:13}}>Carregando...</p>
    </div>
  );
  if(!user) return <LoginScreen onAuth={u=>setUser(u)}/>;
  if(!coupleId) return <CoupleSetup userId={user.id} onReady={id=>{setCoupleId(id);setSyncedOnce(false);}}/>;

  return(<div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",minHeight:"100dvh",background:LP.cream,fontFamily:"'Lora',serif",position:"relative",transition:"background 0.3s",WebkitTapHighlightColor:"transparent",touchAction:"pan-y"}}>
    <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,700;0,900;1,400&family=Lora:ital,wght@0,400;0,500;1,400&family=Cormorant+Garamond:ital,wght@0,400;0,500;1,400;1,500;1,600&display=swap" rel="stylesheet"/>
    ...
    {!selectedDay&&(<>
      ...
      {tab==="home"&&<div style={{paddingBottom:"calc(80px + env(safe-area-inset-bottom))"}}>
        ...
        <div style={{background:`linear-gradient(160deg,${LP.burgundyDeep} 0%,${LP.burgundy} 60%,#8B3A5A 100%)`,padding:"48px 24px 22px",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{textAlign:"center",position:"relative"}}>
            ...
            <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:28,color:"#fff",fontWeight:900,margin:"0 0 3px",lineHeight:1.1}}>{profile.he&&profile.she?`${profile.he} & ${profile.she}`:"2emCristo"}</h1>
        ...
        <div style={{paddingTop:12,display:"flex",flexDirection:"column",gap:8}}>
          ...
          <PrayerIntentionsList P={P} intentions={prayerIntentions} setIntentions={setPrayerIntentions}/>
        </div>
      ...
      {/* JOURNEY */}
      {tab==="journey"&&(()=>{
        const coupleType=profile?.coupleType||"casados";
        const isNoivos=coupleType==="noivos";
        const isNamorados=coupleType==="namorados";
        const activeDays=isNamorados?NAMORADOS_DAYS:isNoivos?NOIVOS_DAYS:DAYS;
        ...
        <div style={{paddingBottom:"calc(80px + env(safe-area-inset-bottom))"}}>
          <div style={{background:`linear-gradient(160deg,${LP.burgundyDeep},${LP.burgundy})`,padding:"52px 24px 24px",textAlign:"center"}}>
            ...
            <div style={{fontSize:10,color:LP.goldLight,letterSpacing:3,fontFamily:"'Lora',serif",textTransform:"uppercase",marginTop:8}}>
              {isNamorados?"Jornada dos Namorados":isNoivos?"Preparação para o Matrimônio":"8 Dias de Renovação"}
            </div>
            <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:26,color:"#fff",fontWeight:900,margin:"4px 0"}}>
              {isNamorados?"Discernimento a Dois":isNoivos?"Jornada dos Noivos":"Propósitos Bíblicos"}
            </h2>
          ...
          <div style={{paddingTop:18}}>
            {activeDays.map(day=>(
              <div key={day.id} onClick={()=>setSelectedDay(day)} style={{margin:"0 16px 10px",borderRadius:16,background:day.id===currentDay?`${day.color}18`:LP.card,border:`1.5px solid ${day.id===currentDay?day.color:LP.gold+"22"}`,cursor:"pointer",boxShadow:day.id===currentDay?`0 4px 20px ${day.color}33`:`0 2px 8px ${LP.shadow}`}}>
                <div style={{display:"flex",alignItems:"center",padding:"14px 16px",gap:14}}>
                  ...
                  <div style={{flex:1}}>
                    <div style={{fontSize:10,color:LP.textMuted,fontFamily:"'Lora',serif",letterSpacing:1.5,textTransform:"uppercase"}}>{day.label}</div>
                    <div style={{fontSize:16,fontFamily:"'Playfair Display',serif",color:LP.burgundy,fontWeight:700,lineHeight:1.2}}>{day.sublabel}</div>
                    <div style={{fontSize:11,color:LP.textMuted,fontFamily:"'Lora',serif",marginTop:2}}>{day.theme}</div>
                  </div>
            ...
            <div style={{margin:"8px 16px 24px",borderRadius:18,overflow:"hidden",border:`2.5px solid ${allDone?"#2E5E3A":LP.gold+"33"}`,boxShadow:allDone?"0 8px 28px #2E5E3A44":"none",transition:"all 0.5s ease"}}>
              ...
              <div style={{background:LP.card,padding:"16px 18px"}}>
                {!allDone?(
                  <>
                    <div style={{display:"flex",gap:4,marginBottom:12}}>
                      {activeDays.map(d=><div key={d.id} style={{flex:1,height:5,borderRadius:3,background:completedDays.includes(d.id)?d.color:LP.gold+"22",transition:"background 0.4s"}}/>)}
                    </div>
                    <p style={{margin:0,fontFamily:"'Lora',serif",fontSize:12,color:LP.textMuted,lineHeight:1.7}}>
                    ...
                    <div style={{marginTop:8,fontSize:11,color:LP.textMuted,fontFamily:"'Lora',serif",fontStyle:"italic"}}>{completedDays.length} de 8 dias concluídos</div>
                  </>
                ):(
                  <>
                    <div style={{marginBottom:14,padding:"12px 14px",borderRadius:12,background:"#2E5E3A11",border:"1px solid #2E5E3A33"}}>
                      <p style={{margin:0,fontFamily:"'Playfair Display',serif",fontSize:14,color:LP.text,lineHeight:1.7,fontStyle:"italic"}}>"Combati o bom combate, acabei a carreira, guardei a fé."</p>
                      <div style={{fontSize:10,color:LP.textMuted,fontFamily:"'Lora',serif",marginTop:4,textAlign:"right"}}>— 2 Tm 4,7</div>
                    </div>
                    <p style={{margin:"0 0 16px",fontFamily:"'Lora',serif",fontSize:12,color:LP.textMuted,lineHeight:1.7}}>
      ...
      {/* Back arrow — visible on non-home tabs */}
      {tab!=="home"&&(
        <button onClick={()=>setTab("home")} style={{
          position:"fixed",top:16,left:16,zIndex:45,
          background:`${LP.burgundy}CC`,backdropFilter:"blur(10px)",
          border:`1px solid ${LP.gold}44`,borderRadius:12,
          padding:"7px 13px",cursor:"pointer",
          display:"flex",alignItems:"center",gap:6,
          boxShadow:"0 3px 12px #0003",
        }}>
          <span style={{fontSize:14,color:"#fff",lineHeight:1}}>←</span>
          <span style={{fontSize:10,color:LP.goldLight,fontFamily:"'Lora',serif",letterSpacing:1}}>Início</span>
        </button>
      )}
